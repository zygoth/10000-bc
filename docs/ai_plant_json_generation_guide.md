...
- `drainage` (object, required)
  - `tolerance_range` (array of 2 floats `[min, max]` in [0.0, 1.0])
  - `min` = minimum tolerated tile drainage index
  - `max` = maximum tolerated tile drainage index
  - **Strict germination rule:** if tile drainage index is outside this range, germination chance is 0.
- `fertility` (object, required)
  - `tolerance_range` (array of 2 floats `[min, max]` in [0.0, 1.0])
  - `min` = minimum tolerated tile fertility index
  - `max` = maximum tolerated tile fertility index
  - **Strict germination rule:** if tile fertility is outside this range, germination chance is 0.
- `moisture` (object, required)
  - `tolerance_range` (array of 2 floats `[min, max]` in [0.0, 1.0])
  - `min` = minimum tolerated tile moisture
  - `max` = maximum tolerated tile moisture
  - **Strict germination rule:** if tile moisture is outside this range, germination chance is 0.
- `shade` (object, required)
  - `tolerance_range` (array of 2 floats `[min, max]` in [0.0, 1.0])
...
**`on_tile_entry_injury`** (object or null, optional — omit if not applicable)
- Fires every time the player steps onto a tile containing this plant
- Omit or set to `null` for non-hazardous plants
- Three injury types: `"scratch"`, `"sting"`, `"contact_rash"`
- **`scratch`** and **`sting`** deal an instant health hit; `"scratch"` also supports an optional `infection_chance`
- **`contact_rash`** uses a `debuff` object for slow health drain over days (no instant `health_hit`); treated by partner with `antihistamine_poultice`
- Equipment modifier slots: `"gloves"` (0.0 = fully negated) and `"coat"` (0.5 = halved)

```json
"on_tile_entry_injury": {
  "type": "scratch",
  "base_probability": 0.35,
  "health_hit": 0.03,
  "infection_chance": 0.10,
  "debuff": null,
  "equipment_probability_modifiers": { "gloves": 0.0, "coat": 0.5 }
}
```

For `contact_rash`, `health_hit` is `null` and `debuff` is populated:
...
"on_tile_entry_injury": {
  ...
  "debuff": {
    "health_drain_per_day": 0.03,
    "duration_days": [4, 7],
    "treatment_tag": "antihistamine_poultice"
  },
  "equipment_probability_modifiers": { "gloves": 0.2, "coat": 0.5 }
}
```

**Equipment modifier guidance:**
- `"gloves": 0.0` — any scratch or sting where hands/forearms are the primary contact point
- `"gloves": 0.2–0.4` — contact rash plants where legs and torso are also exposure vectors (e.g., poison ivy)
- `"coat": 0.5` — thorns or stinging plants at torso/arm height
- `"coat": 0.3` — very dense shrubs
- Both slots are only relevant on `on_tile_entry_injury`; `on_harvest_injury` uses `tool_probability_modifiers` instead

**Representative values for common hazardous plants:**

| Plant | Type | `base_probability` | `health_hit` |
|-------|------|--------------------|--------------|
| Honey locust (tile entry) | scratch | 0.35 | 0.03 |
| Hawthorn (tile entry) | scratch | 0.45 | 0.04 |
| Wild rose (tile entry) | scratch | 0.30 | 0.02 |
| Stinging nettle (tile entry) | sting | 0.60 | 0.02 |
| Poison ivy (tile entry) | contact_rash | 0.50 | null |
...
- **CRITICAL - Tree Growth Timelines:** Trees (woody perennials) should have realistic multi-year growth periods. Use these guidelines:
  - **Fast-growing trees** (willow, cottonwood, honey locust): Seedling stage 200 days (5 years), reach mature canopy size (typically 8-10) by 280-320 days (7-8 years)
  - **Medium-growth trees** (oak, maple, ash): Seedling stage 240 days (6 years), reach mature canopy size (typically 8-10) by 320-360 days (8-9 years)
...
"life_stages": [
  ...
  {"stage": "vegetative", "min_age_days": 2, "seasonal_window": {"start_day": 1, "end_day": 10}, "size": 3, "field_description": "..."},
  {"stage": "flowering", "min_age_days": 3, "seasonal_window": {"start_day": 11, "end_day": 20}, "size": 3, "field_description": "..."},
  {"stage": "seed_set", "min_age_days": 4, "seasonal_window": {"start_day": 21, "end_day": 30}, "size": 3, "field_description": "..."},
...
"life_stages": [
  {"stage": "seedling", "min_age_days": 0, "seasonal_window": null, "size": 1, "field_description": "..."},
  {"stage": "vegetative", "min_age_days": 4, "seasonal_window": {"start_day": 1, "end_day": 15}, "size": 4, "field_description": "..."},
  {"stage": "flowering", "min_age_days": 5, "seasonal_window": {"start_day": 16, "end_day": 25}, "size": 4, "field_description": "..."},
  {"stage": "fruiting", "min_age_days": 6, "seasonal_window": {"start_day": 26, "end_day": 35}, "size": 4, "field_description": "..."},
...
- **Every `outputs[].part` MUST exactly match a `parts[].name` defined in the same `plant.json`**
...
- Runtime applies vitality loss **per harvest action**:
...
- When this cap is reached, next depletion is final (no additional lump vitality hit; vitality has already been applied incrementally per action)
...
- Same shape as `on_tile_entry_injury` but fires per harvest action on this sub-stage
- Uses **`tool_probability_modifiers`** (tool id → float multiplier) instead of `equipment_probability_modifiers`
- Omit entirely for non-hazardous parts
...
"on_harvest_injury": {
  ...
  "infection_chance": null,
  "debuff": null,
  "tool_probability_modifiers": { "knife": 0.4, "gloves": 0.0 }
}
```

For `contact_rash` harvest injuries (e.g., wild parsnip leaves), use `health_hit: null` and populate `debuff` the same way as in `on_tile_entry_injury`.

**Representative values for common hazardous parts:**

| Plant | Part | Type | `base_probability` | `health_hit` |
|-------|------|------|--------------------|--------------|
| Honey locust | green pod harvest | scratch | 0.25 | 0.02 |
| Stinging nettle | leaf harvest | sting | 0.70 | 0.02 |
| Wild parsnip | leaf/stem harvest | contact_rash | 0.40 | null |

**`does_blickey_help_harvest`** (bool, optional — default `false`)
- Set `true` on fruit, berry, and small-seed sub-stages
- When `true` and the player has a blickey, `units_per_action` is multiplied by 1.25
- No effect on tick cost

**`can_squirrel_cache`** (bool, optional — default `false`)
- Set `true` on the specific sub-stage a squirrel would actually store
- Can be a directly-harvested part or a processing output (e.g., extracted beans)
- Determines eligibility for squirrel cache tile contents at world-gen
- Set `true` on: walnuts, hickory nuts, acorns, hazelnuts (harvestable nut sub-stage), and honey locust extracted beans
...
| `"antihistamine_poultice"` | Contact rash debuff (halves duration and drain rate); source plants: jewelweed, plantain |
...
- `life_stages[].size` — int, required on every life stage; valid range 1-10
- `scent.strength` — float 0.0-1.0
- `soil.ph_range` — array of 2 floats, valid range [3.0, 9.0]
- `soil.drainage.tolerance_range` — array of 2 floats `[min, max]`, valid range [0.0, 1.0], must satisfy `min <= max`
...
- `on_tile_entry_injury.base_probability` — float 0.0-1.0; `health_hit` float 0.0-1.0 or null; `infection_chance` float 0.0-1.0 or null
- `on_harvest_injury.base_probability` — float 0.0-1.0; same health/infection constraints as above
- `on_tile_entry_injury.equipment_probability_modifiers` — float values 0.0-1.0 per slot; 0.0 = fully negated
- `on_harvest_injury.tool_probability_modifiers` — float values 0.0-1.0 per tool; 0.0 = fully negated
- `debuff.health_drain_per_day` — float, positive; `debuff.duration_days` — array of 2 ints [min, max]
...
11. **Medicinal treatment:** Medicinal `dose_response` bands must have `treatment_tag` to be usable by partner. Contact rash `debuff` objects must have `treatment_tag: "antihistamine_poultice"`.
...
28. **Life-stage size bounds:** Every `life_stages[].size` must be an integer from 1 to 10 (inclusive). Use 1 for minimal seedling/dormant-above-ground footprint; do not use 0.
29. **Drainage tolerance range:** `soil.drainage.tolerance_range` is required and must be exactly two floats `[min, max]` with `0.0 <= min <= max <= 1.0`. Germination must be treated as impossible outside this range.
30. **Fertility tolerance range:** `soil.fertility.tolerance_range` is required and must be exactly two floats `[min, max]` with `0.0 <= min <= max <= 1.0`. Germination must be treated as impossible outside this range.
31. **Moisture tolerance range:** `soil.moisture.tolerance_range` is required and must be exactly two floats `[min, max]` with `0.0 <= min <= max <= 1.0`. Germination must be treated as impossible outside this range.
32. **Shade tolerance range:** `soil.shade.tolerance_range` is required and must be exactly two floats `[min, max]` with `0.0 <= min <= max <= 1.0`. Germination must be treated as impossible outside this range.
33. **Tile entry injury shape:** If `on_tile_entry_injury` is present, it must include `type`, `base_probability`, `health_hit`, `infection_chance`, `debuff`, and `equipment_probability_modifiers`. For `scratch`/`sting`, `health_hit` is a float and `debuff` is `null`. For `contact_rash`, `health_hit` is `null` and `debuff` is a populated object with `health_drain_per_day`, `duration_days` (array of 2 ints), and `treatment_tag: "antihistamine_poultice"`.
...
- `does_blickey_help_harvest` and `can_squirrel_cache` default to `false` and should only be set `true` where applicable — omitting them entirely is equivalent to `false`
...
- **Medicinal bands must include `treatment_tag`** from the enum above (antibacterial_poultice, anti_inflammatory_poultice, fever_tea, tannin_tea, analgesic, or antihistamine_poultice)
...
```json
{
  ...
  "soil": {
    "ph_range": [6.0, 8.0],
    "drainage": {
      "tolerance_range": [0.45, 0.9]
    },
    "fertility": {
      "tolerance_range": [0.4, 0.9]
    },
    "moisture": {
      "tolerance_range": [0.35, 0.85]
    },
    "shade": {
      "tolerance_range": [0.0, 0.3]
    }
  },
  "ph_effect_on_soil": 0.0,
  "dispersal": {
      ...
      "sub_stages": [
        {
          ...
          "on_harvest_injury": {
            ...
            "debuff": null,
            "tool_probability_modifiers": { "gloves": 0.0, "knife": 0.5 }
          },
          "decay_days": 2.0,
          "can_dry": false,
...
2. **Populate top-level fields** based on botanical reality
   ...
   - Add `on_tile_entry_injury` for any plant with thorns, stinging hairs, or contact-rash compounds
...
- **Open-ended fields** (plant_family, taste_notes, scent_notes, texture, physical_description) should be botanically accurate but are not constrained to specific values
...
- Added `on_harvest_injury` sub-stage field: same shape as tile entry injury but fires per harvest action; uses `tool_probability_modifiers` instead of equipment slots
- `contact_rash` injury type uses a `debuff` object (slow health drain over days) instead of an instant `health_hit`; treated by partner with new `antihistamine_poultice` treatment tag
- Added `does_blickey_help_harvest` sub-stage bool: set `true` on fruit/berry/small-seed sub-stages; multiplies `units_per_action` by 1.25 when player has a blickey
- Added `can_squirrel_cache` sub-stage bool: set `true` on the specific sub-stage a squirrel would store (nuts, extracted beans); used in world-gen squirrel cache tile contents
- Added `antihistamine_poultice` to the `treatment_tag` enum (halves contact rash duration and drain rate; source plants: jewelweed, plantain)
